        ...
        <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-5 gap-2 sm:gap-3">
          {FOCUSES.map((f) => (
            <button
              ...
              className={`py-4 sm:py-4 px-2 sm:px-3 rounded-lg border-2 transition-all duration-200 text-center ${
                focus === f.value
                  ? "border-primary-500 bg-primary-50 text-primary-700"
                  : "border-gray-200 hover:border-gray-300 bg-white"
              }`}
      ...
      <div className="bg-gradient-to-r from-emerald-50 to-teal-50 p-3 sm:p-4 rounded-lg border border-emerald-200">
        <div className="flex items-center justify-between gap-3">
          ...
          <button
            ...
            className={`relative inline-flex items-center h-7 w-12 sm:h-6 sm:w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 ${
              useRealContent ? 'bg-emerald-500' : 'bg-gray-300'
            }`}
          >
            <span
              className={`pointer-events-none inline-block h-5 w-5 sm:h-5 sm:w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
